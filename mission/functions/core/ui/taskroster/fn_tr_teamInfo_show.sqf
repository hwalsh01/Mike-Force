...
private _groupMembers = missionNamespace getVariable [_groupID, []];
...
private _traitsTeam = _traitConfigs select {(configName _x) isNotEqualTo "vn_artillery"} apply {
	...
	private _traitPlayerCount = count (
		_groupMembers select {_x getUnitTrait _traitName}
	);
...
private _teamTraits = _groupMembers apply {
	private _player = _x;
...
private _traitsPlayer = _traitConfigs select {
...
call vn_mf_fnc_tr_teamInfo_listboxInit;